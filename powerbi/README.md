...
- [Publish ⇄ export round-trip](#-publish--export-round-trip)
- [Caveats](#-caveats)
...
## 🔁 Publish ⇄ export round-trip

The **report** is authored in the Power BI **Service** (built with **Copilot** from the
published model — Power BI reports aren't generated headlessly) and round-tripped to the repo
as code:

```bash
# pull the working report + model back into powerbi/ (Fabric getDefinition; rebinds byPath
# + genericizes the Databricks host/warehouse)
WORKSPACE_ID=<guid> REPORT_ID=<guid> MODEL_ID=<guid> python tools/export_powerbi.py

# push the PBIP to a workspace (fabric-cicd)
WORKSPACE_ID=<guid> python tools/publish_powerbi.py
```

The committed report is the Copilot‑built page (KPIs, risk‑tier column, at‑risk table,
sole‑source treemap, program slicer) carrying a **NASA theme** (NASA blue `#0B3D91` + red
`#FC3D21`) under `ArtemisSupplyRisk.Report/StaticResources/RegisteredResources/`. To restyle,
edit that theme JSON and re‑publish.

> [!NOTE]
> **Enable once in the Service for Copilot authoring:** the workspace needs a **Fabric
> capacity**, and the semantic model needs **Q&A** + **Approved for Copilot** turned on
> (model → Settings). Then *New report → Copilot* can (re)generate pages.
...
> [!NOTE]
> - The report is exported from the Service, so it's known‑good there. Opening the PBIP in
>   **Desktop** prompts for the 3 Databricks parameters (host / HTTP path / catalog) on first
>   open — supply yours, since the committed values are placeholders.
> - The two fact tables are sliced **independently** by `program` (no relationship); add a
>   shared `Program` dimension if you want one slicer to filter both.
> - The click‑by‑click manual build also lives in
